...
<div class=header><h1>⚽ FOOTY • 130+ РЕАЛЬНЫХ — 52 ЛИГИ — АРХИТЕКТУРА 00:00 МСК — 00:00 МСК КРОН</h1><div class=sub>00:00 МСК — крон → 1. Тяну расписание на завтра с Sofascore API scheduled-events/{target} — 52 лиги — как ты сказал — как ты писал → 2. Для каждого матча: FBref → тактика 35 факторов (PPDA фланги % кроссы точность прогрессивные ширина низкий блок % — Team Stats — сейчас рандом 58-84% надо реальные 78% у Реала), Understat → xG 20 факторов (xG xGA xG кросса xG удара setPiece xG конверсия — рандом 1.8-3.0 надо реальный 2.8 у Реала vs Малаги), Transfermarkt → судья 15 факторов (среднее ЖК строгость home bias фолов/игру — рандом 3.2-5.2 надо реальный реф), OpenWeatherMap → погода 25 факторов (по координатам стадиона Bernabeu 40.4530,-3.6883 ветер м/с дождь мм темп влажность — рандом 2-13 надо реальный прогноз 31 авг 19:00 Idrettsparken 13 м/с + 4мм — вручную для Egersund vs Asane надо автоматом), Flashscore → составы/травмы/мотивация за 1 час до матча если у Реала нет Вини фланги падают 78% до 60% — меняет ТБ на ТМ, H2H → история 15 факторов (H2H угловые среднее ЖК среднее голы среднее — рандом 7.5-11.5 надо реальные 11,12,13 в дерби) → всего 130+ реальных, не рандом → 3. Прогоняю через экспертные связки как Реал vs Малага фланги 78% реально, Egersund vs Asane ветер 13 м/с реально → 4. Сохраняю в JSON и рендерю в твой сайт | Обновлено {now} | {len(enriched)} матчей — 11 реальных на 31 авг: Celta vs Athletic, Osasuna vs Getafe, Barcelona vs Rayo, Lecce vs Roma, Atalanta vs Bologna, Egersund vs Asane ветер 13+4 и т.д. | ESPN fallback 80 матчей | 2 воркфлоу</div></div>
...
<div class=league-head><div><div class=league-title>{m['league']} — {m['home']} vs {m['away']} — {m['kickoff']} — {m['venue']} {m['coords'] or ''}</div><div style="font-size:10px;color:#a1a1aa">🌬️ {m['weather']['wind']} м/с {'✅ РЕАЛЬНАЯ по координатам' if m['weather'].get('real') else '⚠️ рандом 2-13 надо координаты'} | 🌡️ {m['weather']['temp']}°C | Судья {m['referee'] or 'нет'} {'✅ REAL' if f['referee']['real'] else '⚠️ RANDOM 3.2-5.2'} | Дерби {m['isDerby']} {'✅ Sofascore флаг' if m['isDerby'] else ''} | REAL {real_count}/4</div></div><div><span class=badge {'real' if real_count>=2 else 'fake'}>REAL {real_count}/4</span><span class=badge expert>130+ факторов</span></div></div>
